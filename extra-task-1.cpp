...
double to_24_hour_clock(double hours)
{
    assert(hours >= 0);
    double cur_hour;
    double cur_min = modf(hours, &cur_hour);
...
{
    int hours = (time - utc_offset) > 0 ? (time - utc_offset) : 24 + (time - utc_offset);
...
int main(){
    ...
    std::cout << "Tests seconds_difference completed" << std::endl; 
    
    assert(fabs(hours_difference(1800.0, 3600.0) - 0.5) < DBL_EPSILON);
    assert(fabs(hours_difference(3600.0, 1800.0) - (-0.5)) < DBL_EPSILON);
    assert(fabs(hours_difference(1800.0, 2160.0) - 0.1) < DBL_EPSILON);
    assert(fabs(hours_difference(1800.0, 1800.0) - 0.0) < DBL_EPSILON);
    std::cout << "Tests hours_difference completed" << std::endl;

    assert(fabs(to_float_hours(0, 15, 0) - 0.25) < DBL_EPSILON);
    assert(fabs(to_float_hours(2, 45, 9) - 2.7525) < DBL_EPSILON);
    assert(fabs(to_float_hours(1, 0, 36) - 1.01) < DBL_EPSILON);
    std::cout << "Tests to_float_hours completed" << std::endl;
    ...
    std::cout << "Tests time_to_utc completed" << std::endl;
    ...
    std::cout << "Tests time_from_utc completed" << std::endl;
